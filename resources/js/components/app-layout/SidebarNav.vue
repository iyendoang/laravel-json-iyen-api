...
import type { MenuItem, MenuLeaf, MenuParent, MenuDivider } from '@/types/menu'
...
// 🔥 Destructure yang diperlukan saja
const {
  isItemActive, // Universal - bisa cek semua tipe
  isItemOrChildActive, // Untuk auto-expand
  hasAccess, // Cek akses
  isMenuLeaf, // Type guard
  isMenuParent, // Type guard
  isMenuDivider // Type guard
} = useMenu()

// State untuk collapsible groups
    ...
    <SidebarMenu class="gap-0.5">
      <template v-for="item in items" :key="item.title || Math.random()">
        ...
        <div v-if="isMenuDivider(item)" class="border-border/30 my-2 border-t">
          <span
            v-if="item.title"
            class="text-muted-foreground/50 px-3 text-[9px] font-semibold uppercase"
          >
            {{ item.title }}
          </span>
        </div>
        ...
        <SidebarMenuItem v-else-if="isMenuLeaf(item)">
          ...
          >
            ...
            >
              ...
              />
              <span
                class="truncate transition-opacity duration-200 group-data-[state=collapsed]:opacity-0"
              >
                {{ item.title }}
              </span>
            </a>
            ...
            >
              ...
              <span
                v-if="item.badge"
                class="ml-auto rounded-full px-1.5 py-0.5 text-[10px] font-bold text-white"
                :class="`bg-${item.badgeColor || 'red'}-500`"
              >
                {{ item.badge }}
              </span>
            </RouterLink>
        ...
        >
          <SidebarMenuItem>
            <CollapsibleTrigger as-child @click.prevent="toggleGroup(item.title)">
              <SidebarMenuButton
                :tooltip="item.tooltip || item.title"
                :is-active="isItemActive(item)"
